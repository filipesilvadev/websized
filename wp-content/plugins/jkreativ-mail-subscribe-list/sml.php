<?php
/*
Plugin Name: Jkreativ Mail Subscribe List
Plugin URI: http://jegtheme.com
Description: Simple customisable plugin that displays a name/email form where visitors can submit their information, managable in the WordPress admin. Modified to meet Jkreativ Requirement
Version: 1.0.0
Author: Richard Leishman t/a Webforward modified by Jegtheme
...
function smlsubform($atts=array()){
	...
	if (isset($_POST['sml_subscribe']) && $_POST['sml_subscribe'] && $thankyou) { 
		if ($jsthanks) {
	...
	$return .= '<input class="sml_emailinput" name="sml_email" placeholder="'.$emailholder.'" type="text" value="">';
...
wp_register_sidebar_widget(
    'sml_subscribe_widget',
    'Subscribe Form (Vertical Navigation Widget)',
...
if (isset($_POST['sml_subscribe'])) {
	...
	if (is_email($email)) {
		...
		if (mysql_num_rows($exists) <1) {
			$wpdb->query("insert into ".$wpdb->prefix."sml (sml_email) values ('".$wpdb->escape($email)."')");
		}
	}
}
