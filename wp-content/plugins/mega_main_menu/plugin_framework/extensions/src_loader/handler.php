 ...
 */
	...
	function mmpm_load_all_src ( $args ) {
		...
		if ( is_admin() ) { // Sources for wp-admin
            ...
            if($currentscreen->id === "appearance_page_mmpm_options_mega_main_menu" || $currentscreen->id === 'nav-menus') {
		...
		foreach ( $page_all_js as $key => $value ) {
			if ( $value != '' ) {
				wp_register_script( $key, $value, 'jquery' );
			}
			wp_enqueue_script( $key , null, null, null, true );
		}
	}
?>