 ...
 */
	function mmpm_theme_options_array(){
		foreach ( get_nav_menu_locations() as $key => $value ){
			if($key !== 'side_navigation' && $key !== 'top_navigation') continue;
			$key = str_replace( ' ', '-', $key );
		...
		foreach ( get_nav_menu_locations() as $key => $value ){
			...
			$locations_options = array_merge( 
				$locations_options, array(
					array(
						'name' =>  __( 'Layout Options: ', MMPM_TEXTDOMAIN_ADMIN ) . '&nbsp; <strong>' . $key . '</strong>',
					...
					array(
						...
						'values' => array(
							...
							__( 'WooCart (on right side)', MMPM_TEXTDOMAIN_ADMIN ) => 'woo_cart',
						),
						'default' => array(),
					),
					array(
					...
					array(
						...
						'values' => array(
							...
							__( 'Right', MMPM_TEXTDOMAIN_ADMIN ) => 'right',
							__( 'Disable Icons In First Level Items', MMPM_TEXTDOMAIN_ADMIN ) => 'disable_first_lvl',
							__( 'Disable Icons Globally', MMPM_TEXTDOMAIN_ADMIN ) => 'disable_globally',
						),
						'default' => array( 'disable_first_lvl', ),
					),
					array(
...
/*
					array(
						'name' => __( 'Full Width Initial Container', MMPM_TEXTDOMAIN_ADMIN ),
						'descr' => __( 'If this option is enabled then the primary container will try to be the full width.', MMPM_TEXTDOMAIN_ADMIN ),
...
*/
					array(
						...
						'descr' => __( 'Check this option to make the menu sticky. Not working on mobile devices.', MMPM_TEXTDOMAIN_ADMIN ),
					...
					array(
						'name' => __( 'Forced PHP Integration', MMPM_TEXTDOMAIN_ADMIN ),
						'descr' => __( 'If you have knowledge of PHP you can call this function anywhere in order to display this menu. Please use it only if you professional.', MMPM_TEXTDOMAIN_ADMIN ),
						'key' => 'forced_integration',
						'type' => 'just_html',
						'default' => '<pre>&lt;?php wp_nav_menu( array( "theme_location" => "' . $original_menu_slug . '" ) ); ?&gt;</pre>',
					),
					array(
		...
		$locations_options = array_merge( 
			$locations_options, array(
				...
				array(
					...
					'key' => 'backup',
					'type' => 'just_html',
					'default' => '<a href="' . get_admin_url() . '?mmpm_page=backup_file">' . __( 'Download backup file with current settings') . '</a><br /><br />' . __( 'Upload backup file and restore settings. Chose file and click "Save All Settings".') . '<br /><input class="col-xs-12 form-control input-sm" type="file" name="' . MMPM_OPTIONS_DB_NAME . '_backup" />',
				),
			) // 'options' => array
		);
		...
		foreach ( get_nav_menu_locations() as $key => $value ){
			...
			$skins_options = array_merge( 
				$skins_options, array(
					array(
						'name' =>  __( 'Skin Options: ', MMPM_TEXTDOMAIN_ADMIN ) . '&nbsp; <strong>' . $key . '</strong>',
						'key' => $key . '_menu_skin',
						'type' => 'collapse_start',
					),
					array(
						'name' => __( 'Skin Options of the Initial Container', MMPM_TEXTDOMAIN_ADMIN ),
					...
					array(
						'name' => __( 'Background image of the primary container', MMPM_TEXTDOMAIN_ADMIN ),
					...
					array(
						'name' => __( 'Skin Options of the First Level Items', MMPM_TEXTDOMAIN_ADMIN ),
						'key' => 'dropdowns_skin_options',
						'type' => 'caption',
					),
					array(
					...
					array(
						'name' => __( 'Skin Options of the Dropdowns', MMPM_TEXTDOMAIN_ADMIN ),
						'key' => 'dropdowns_skin_options',
						'type' => 'caption',
					),
					array(
					...
					array(
						'name' => __( 'Plain Text Color of the Dropdown', MMPM_TEXTDOMAIN_ADMIN ),
...
*/
			array(
				...
				'options' => array(
					...
					array(
						...
						'descr' => __( '<span style="color: #f11;">Warning:</span> If this option is checked - all menus will be replaced by the mega menu. This will be useful only for templates in which are not defined locations of the menu and template has only one menu.', MMPM_TEXTDOMAIN_ADMIN ),
			...
			array(
				...
				'options' => array(
					array(
						'name' => __( 'Here you can deactivate the options that you  do not use to customize the menu structure. This helps reduce the number of options and reduce the load on the server.', MMPM_TEXTDOMAIN_ADMIN ),
						'key' => 'menu_structure_settings',
						'type' => 'caption',
					),
					array(
						'name' => __( 'Style of the item', MMPM_TEXTDOMAIN_ADMIN ),
						'key' => 'item_style',
						'type' => 'checkbox',
						'values' => array(
							__( 'Disable', MMPM_TEXTDOMAIN_ADMIN ) => 'disable',
						),
					),
					array(
						'name' => __( 'Icon of the item', MMPM_TEXTDOMAIN_ADMIN ),
						'key' => 'item_icon',
						'type' => 'checkbox',
						'values' => array(
							__( 'Disable', MMPM_TEXTDOMAIN_ADMIN ) => 'disable',
					...
					array(
						'name' => __( 'Hide Icon of the Item', MMPM_TEXTDOMAIN_ADMIN ),
						'key' => 'disable_icon',
						'type' => 'checkbox',
						'values' => array(
							__( 'Disable', MMPM_TEXTDOMAIN_ADMIN ) => 'disable',
					...
					array(
						'name' => __( 'Hide Text of the Item', MMPM_TEXTDOMAIN_ADMIN ),
						'key' => 'disable_text',
						'type' => 'checkbox',
						'values' => array(
							__( 'Disable', MMPM_TEXTDOMAIN_ADMIN ) => 'disable',
					...
					array(
						'name' => __( 'Disable Link', MMPM_TEXTDOMAIN_ADMIN ),
						'key' => 'disable_link',
						'type' => 'checkbox',
						'values' => array(
							__( 'Disable', MMPM_TEXTDOMAIN_ADMIN ) => 'disable',
					...
					array(
						'name' => __( 'Submenu Type', MMPM_TEXTDOMAIN_ADMIN ),
						'key' => 'submenu_type',
						'type' => 'checkbox',
						'values' => array(
							__( 'Disable', MMPM_TEXTDOMAIN_ADMIN ) => 'disable',
					...
					array(
						'name' => __( 'Side of dropdown elements', MMPM_TEXTDOMAIN_ADMIN ),
					...
					array(
						'name' => __( 'Submenu Columns', MMPM_TEXTDOMAIN_ADMIN ),
						'key' => 'submenu_columns',
						'type' => 'checkbox',
						'values' => array(
							__( 'Disable', MMPM_TEXTDOMAIN_ADMIN ) => 'disable',
					...
					array(
						'name' => __( 'Enable Full Width Dropdown', MMPM_TEXTDOMAIN_ADMIN ),
