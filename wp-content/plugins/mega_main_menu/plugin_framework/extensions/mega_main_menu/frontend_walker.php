...
//            $item_output .= $link_before;
            $item_output .= '<span>';
            $item_output .= '<span class="link_text">' . $item->title . '</span>';
            $item_output .= '</span>';
...
*/
        ...
        function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if((string)$depth == '0') {
				$paddingright 	= get_post_meta( $args->menu_main_parent, MMPM_PREFIX . '_submenu_additional_right_margin', true);
				$paddingbottom 	= get_post_meta( $args->menu_main_parent, MMPM_PREFIX . '_submenu_additional_bottom_margin', true);
				
				if(!empty($paddingright)) {
					$paddingrightstyle = " padding-right: {$paddingright}px; ";
				}	
				
				if(!empty($paddingbottom)) {
					$paddingbottomstyle = " padding-bottom: {$paddingbottom}px; ";
				}
			}			
			...
			$img = ( (string)$depth == '0' && get_post_meta( $args->menu_main_parent, MMPM_PREFIX . '_submenu_bg_image', true) ) 
                ...
                : 'no-img';				
            $background = ( is_array( $img ) && $img['background_image'] != '') ? ' background-image:url(' . $img['background_image'] . ');background-repeat:' . $img['background_repeat'] . ';background-attachment:' . $img['background_attachment'] . ';background-position:' . $img['background_position'] . ';background-size:' . $img['background_size'] . ';': '';
			
			$style = ' style="' . $paddingrightstyle . $paddingbottomstyle . $background . '" ';
