...
;jQuery(document).ready(function(){
	 ...
	 */
	;jQuery('#update-nav-menu .button-primary, #mmpm_theme_options_form .button-primary').click(function( event ) {
//		event.preventDefault();
		;jQuery('.multiplied_example').remove();
		;jQuery('#menu-to-edit > li:not(.menu-item-depth-0) div[class*="submenu_"]').remove();
//		;jQuery('#update-nav-menu, #mmpm_theme_options_form').submit();
	});
...
(function($){
    ...
    $(document).ready(function(){
        $('body').on('click', '.select_file_button', function(e){
            var element = $(this);
            var input = $(this).parents('.input-group').find('input.upload');
            var preview = $(this).parents('.input-group').next().find('img.img_preview');
